...
class VideoEncoder(nn.Module):
    ...
    def __init__(self, config=None):
        ...
        self.channels = self.config.VIDEO_ENCODER_CHANNELS
        self.feature_dim = self.config.VIDEO_ENCODER_FEATURE_DIM
        self.dropout = self.config.DROPOUT

        self.conv_layers = nn.ModuleList([
            nn.Conv2d(1, self.channels[0], kernel_size=3, stride=(2, 2), padding=1),
            nn.Conv2d(self.channels[0], self.channels[1], kernel_size=3, stride=(2, 2), padding=1),
            nn.Conv2d(self.channels[1], self.channels[2], kernel_size=3, stride=(2, 2), padding=1)
        ])
        ...
        self.fc_layers = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(self.channels[-1] * 8 * 8, self.feature_dim),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T):
            ...
            video_features_list.append(frame)

        video_features = torch.stack(video_features_list, dim=1)
    ...
    def _encode_frame(self, frame: torch.Tensor) -> torch.Tensor:
        ...
        for conv, bn in zip(self.conv_layers, self.bn_layers):
            frame = bn(F.relu(conv(frame)))

        frame = self.fc_layers(frame)
